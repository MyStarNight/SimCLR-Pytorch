            ...
            with torch.no_grad():
                ...
                for batch, (data, target) in enumerate(train_data):
                    ...
                    if epoch == args.max_epoch:
                        prediction_list = prediction[:,0].cpu().numpy().tolist()
                        pred_list.extend(prediction_list)
                        target_list = target.cpu().numpy().tolist()
                        true_list.extend(target_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epoch', default=5, type=int, help='')
